...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ============================================================
  // 8. CHECKOUT — BUY NOW BUTTONS
  // ============================================================
  // CUSTOMIZE: Update product info for your items
  const productInfo = {
    'product-1': { title: 'Product Name', price: '$99' },
    'product-2': { title: 'Product Name', price: '$149' },
    'product-3': { title: 'Product Name', price: '$249' }
  };
        ...
        <div class="modal__body">
          ...
          <form id="orderForm" style="display:flex;flex-direction:column;gap:0.8rem;margin-top:1rem">
            ...
            <div style="font-size:0.8rem;color:var(--color-text-muted);padding:0.5rem 0">
              📦 We'll send you a payment link within 24 hours.
            </div>
          ...
          <div id="orderSuccess" style="display:none;padding:1rem;background:#E8F5E9;color:#2E7D32;border-radius:0;font-size:0.95rem;margin-top:1rem">
            ✅ Order received! We'll email you within 24 hours.
          </div>
        </div>
      </div>
    </div>`;
  ...
  document.querySelectorAll('.buy-btn').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      e.preventDefault();
      // Generate a product key based on the card's index
      var cards = document.querySelectorAll('.product-card');
      var idx = Array.from(cards).indexOf(this.closest('.product-card'));
      var key = 'product-' + (idx + 1);
  ...
  coForm.addEventListener('submit', function(e) {
    ...
    // CUSTOMIZE: Change the email address below to your own
    var yourEmail = 'your@email.com';
    var subj = encodeURIComponent('Order: ' + product);
    var body = encodeURIComponent(
      'Product: ' + product + '\nPrice: ' + price + '\nName: ' + name + '\nEmail: ' + email + '\nAddress: ' + address
    );
    window.open('mailto:' + yourEmail + '?subject=' + subj + '&body=' + body, '_blank');
    this.style.display = 'none';
    coSuccess.style.display = 'block';
  });

  console.log('✦ Ethereal — Timeless beauty, thoughtfully made.');
});
